...
function velocity = overland(TSP2D,deltaT,s_dep,s_grid,man_area,dem,oflow,x,y,xx,yy,of_rate,varargin)

s0 = (dem(x,y) - dem(xx,yy))/s_grid;
slop_wl = ((oflow(xx,yy) + of_rate(xx,yy)  * deltaT) - (oflow(x,y) + of_rate(x,y) * deltaT))/s_grid;
slop_fic = s0 - slop_wl;
pos_oflow = oflow(x,y)+ of_rate(x,y) * deltaT;
rman = man_area(x,y);
deps = s_dep(x,y);
rate = of_rate(x,y);
if slop_fic < 0
    pos_oflow = oflow(xx,yy) + of_rate(xx,yy) * deltaT;
    rman = man_area(xx,yy);
    deps = s_dep(xx,yy);
    rate = of_rate(xx,yy);
end
...
if dem(x,y) == 999 && dem(xx,yy) == 999
    ...
    if pos_oflow <= deps || slop_fic == 0
        ...
        if abs(slop_fic) >= 0.1
            if slop_fic >= 0
                dqq = 0.6 * s_grid * (h_ofm^(3/2));   % c = 1
            else
                dqq = - 0.6 * s_grid * (h_ofm^(3/2));
            end
        else
            if slop_fic >= 0
...
elseif dem(x,y) < 999 && dem(xx,yy) < 999  
    ...
    if pos_oflow <= deps || slop_fic == 0
        ...
        if abs(slop_fic) >= 0.1
            if slop_fic >= 0
                dqq = 0.6 * s_grid * (h_ofm^(3/2));   % c = 0.6
            else
                dqq = - 0.6 * s_grid * (h_ofm^(3/2));
            end
        else
